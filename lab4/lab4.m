...
xl(2)   = -0.5;             % Travel rate lower bound
xu(2)   = 0.5;              % Travel rate upper bound
%xl(6)   = -0.04;            % Elevation rate lower bound
%xu(6)   = 0.04;             % Elevation rate upper bound
...
R1 = zeros(mu,mu);
R1(1,1) = 10;                            % q1 in lab manual
R1(2,2) = 10;                            % q2 in lab manual
...
options = optimoptions('fmincon', 'Algorithm', 'sqp','MaxFunEvals',500000); % Set SQP algorithm

costFunc = @(z) 0.5*z'*Q*z;


tic
[z,lambda] = fmincon(costFunc , z0, [], [], Aeq, beq, vlb, vub, @(z)nonlincon(z, alfa, beta, lambda_t, mx, N), options);
t1=toc;
...
u1 = [z(N*mx+1:mu:N*mx+M*mu);0];
u2 = [z(N*mx+2:mu:N*mx+M*mu);0];
...
display(t(end));
log_name_order = ["time [s]" "Travel [rad]" "Travel rate [rad]" "pitch [rad]" "pitch rate [rad]" "elevation [rad]" "elevation rate [rad]" "u [2x1](input)" "e_ref [rad]" "Vd" "Vs" "Vf" "Vb"];
...
%Plot this with e?
[c, ceq] = nonlincon(z, alfa, beta, lambda_t, mx, N);
c  = [0;zero_padding; c; zero_padding];
ec=c+x5;
...
function [c, ceq] = nonlincon(z, alfa, beta, lambda_t, nx, N)
    c = zeros(N,1);
    ceq = [];
    for i=1:N
        c(i) = alfa * exp(-beta*(z(1+(i-1)*nx)-lambda_t)^2)-z(5+(i-1)*nx);
    end
end
